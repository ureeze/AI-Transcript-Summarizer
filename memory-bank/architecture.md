...
Frontend
  ...
  - 기본 URL 형식을 검증한다.
  - 요약 요청 상태와 결과를 표시한다.
  - 자막 추출 실패 시 transcript 직접 입력 fallback을 제공한다.
...
Backend
  ...
  - YouTube 자막 추출 실패 시 transcript 직접 입력 기반 요약 확장을 수용할 수 있어야 한다.
...
자막 추출 실패 fallback 흐름:

1. 사용자가 유튜브 URL로 요약을 요청한다.
2. 백엔드가 자막 추출을 시도한다.
3. 자막 추출이 실패하면 프론트엔드가 transcript 직접 입력 fallback을 안내한다.
4. 사용자가 transcript를 직접 입력하면 백엔드는 해당 transcript로 요약을 진행한다.
5. 프론트엔드가 요약 결과를 동일한 결과 화면에 표시한다.
...
- YouTube 자막 추출 실패 시 사용자가 transcript를 직접 입력해 흐름을 이어갈 수 있도록 fallback 전략을 고려한다.
