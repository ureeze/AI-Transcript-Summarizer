...
- [T-074] 로컬/배포 환경에는 실제 `OPENAI_API_KEY`를 설정해야 한다.
...
1. [T-068] Git/PR 및 작업 승인 운영 규칙 변경 PR을 검토하고 `develop`에 병합한다.
2. [T-069] 실제 `OPENAI_API_KEY`를 설정하고 로컬 전체 요약 흐름을 검증한다.
3. [T-070] 프론트엔드 에러 메시지를 API 응답 메시지 기반으로 개선한다.
